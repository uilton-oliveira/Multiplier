ADDON_NAME = 'Power Multiplier'
factor = #FACTOR_HERE# 
divValue = #DIV_VALUE_HERE#

local initlog = false
function Log(msg)
    if initlog == false then
        initlog = true
        InitLogFile( "log/powermultiplier.txt","")
    end
    local lmsg = '[x'..factor..' => LUA] '..msg
    print ( lmsg )
    AppendToLogFile( "log/powermultiplier.txt", lmsg .. "\r\n")
end
...
function PrintObject(t, ident, done)
    PrintTable(getmetatable(t), ident, done)
end


function LogTable(t, indent, done)
    --print ( string.format ('PrintTable type %s', type(keys)) )
    if type(t) ~= "table" then return end
    ...
    for k, v in ipairs(l) do
        ...
        if v ~= 'FDesc' then
            ...
            if type(value) == "table" and not done[value] then
                ...
                Log(string.rep ("\t", indent)..tostring(v)..":")
                PrintTable (value, indent + 2, done)
            elseif type(value) == "userdata" and not done[value] then
                done [value] = true
                Log(string.rep ("\t", indent)..tostring(v)..": "..tostring(value))
                PrintTable ((getmetatable(value) and getmetatable(value).__index) or getmetatable(value), indent + 2, done)
            else
                if t.FDesc and t.FDesc[v] then
                    Log(string.rep ("\t", indent)..tostring(t.FDesc[v]))
                else
                    Log(string.rep ("\t", indent)..tostring(v)..": "..tostring(value))
                end
            end
...
function LogObject(t, ident, done)
    LogTable(getmetatable(t), ident, done)
end
